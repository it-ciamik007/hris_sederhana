...
export async function submitLeaveRequest(leaveRequestId: string) {
  const leave = await db.leaveRequest.findUnique({
    ...
    include: { employee: { include: { supervisor: true, position: true } }, leaveType: true }
  });
  ...
  const firstApproverType = leave.employee.position?.isSpvLevel
    ? "MANAGER"
    : leave.employee.supervisorId
      ? "DIRECT_SPV"
      : "HRD";
    ...
    steps: [
      {
        ...
        approverEmployeeId: firstApproverType === "DIRECT_SPV" ? leave.employee.supervisorId ?? undefined : undefined,
  ...
  const approver = await resolveApproverContact({
    companyId: leave.employee.companyId,
    approverType: firstApproverType,
    approverEmployeeId: firstApproverType === "DIRECT_SPV" ? leave.employee.supervisorId : undefined,
    approverRoleCode: firstApproverType === "MANAGER" ? "MANAGER" : firstApproverType === "HRD" ? "HRD" : undefined
  });
